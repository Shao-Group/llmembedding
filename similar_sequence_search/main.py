...
from embed_bert import bert_embedding
    ...
    def load_dist(self):
        ...
        if not os.path.isfile(idx_dir + "train_dist.npy"):
            ...
            for i in range(self.nq):
                avg += self.query_dist[i][self.query_knn[i][0]]
                for j in range(10):
                    avg10 += self.query_dist[i][self.query_knn[i][j]]

            print(avg/self.nq)
            print(avg10/self.nq/10)
...
def get_args():
    ...
    parser.add_argument(
        "--model-dir", type=str, default="", help="bert pretrained model location"
    )
...
def run_from_train(args, h, data_file):
    ...
    if args.embed == "cnn":
        ...
        xq, xb, xt = bert_embedding(args, h, args.model_dir)
    elif args.embed == "evo":
        xq, xb, xt = evo_embedding(args, h, args.model_dir)
    elif args.embed == "nt":
        xq, xb, xt = nt_embedding(args, h, args.model_dir)
    elif args.embed == "ts":
        ts_embedding(args, h, args.model_dir)
    else:
        assert False
